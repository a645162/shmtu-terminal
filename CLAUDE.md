...
# TUI 构建 (需 ftxui, 通过 vcpkg 安装)
cmake -B build-tui -S . -DBUILD_TUI=ON -DBUILD_SERVER=OFF -DBUILD_GUI=OFF \
    -DCMAKE_TOOLCHAIN_FILE=$VCPKG_ROOT/scripts/buildsystems/vcpkg.cmake
cmake --build build-tui --target shmtu_cas_ocr_tui
# TUI 运行
./build-tui/ocr/shmtu-cas-ocr-tui/shmtu_cas_ocr_tui --help
./build-tui/ocr/shmtu-cas-ocr-tui/shmtu_cas_ocr_tui --version
./build-tui/ocr/shmtu-cas-ocr-tui/shmtu_cas_ocr_tui  # 启动交互界面 (q=退出, r=刷新, d=下载)
# 更新 docker-runtime 二进制
cp build-tui/ocr/shmtu-cas-ocr-tui/shmtu_cas_ocr_tui-*.0 docker-runtime/shmtu_cas_ocr_tui
```
...
- OCR Server 新增 **FTXUI TUI 工具** (`shmtu_cas_ocr_tui`，默认开启)：交互式终端界面，左面板浏览 GitHub Release 标签 + 模型计数，中间面板展示模型指标表格 (backbone/params/val/test acc)，右侧下载面板带进度条；`--model-dir` 覆写缓存目录；通过 vcpkg 的 ftxui 库静态链接
- CI Workflow (`.github/workflows/build-system-vulkan.yml`) 采用 CPU/Vulkan 并行构建 → GHCR 推送 → DockerHub/阿里云并行分发模式
- CI Workflow (`.github/workflows/release-tui.yml`) TUI 跨平台构建 (Ubuntu/Windows/macOS) 并上传到 GitHub Release
...
- **OCR 模型 v1/v2 共存**：4 端推理（Tauri/桌面/Android/C++ Server）均支持 v1（3 模型 ResNet，legacy）与 v2（单模型 MobileNetV3 Tri-Slot Decoder，默认）。默认 v2，UI/CLI 暴露切换项；v2 通过 release 根目录的 `model-assets.json` 智能下载（tag + backbone + precision + engine 维度），v1 沿用硬编码文件列表 + SHA256SUMS.txt 校验。详细对比与切换方式见 [Documents/docs/ocr-model-versions.md](Documents/docs/ocr-model-versions.md)。
...
以下项目的 `Documents/` 内已经搭建 **VitePress** 站点并配置了 **GitHub Pages** 部署 workflow（模仿 `shmtu-terminal-tauri` + `shmtu-cas-rs` 的范式）：

| 项目 | 文档目录 | 部署触发 | 产物路径 |
| --- | --- | --- | --- |
| `Documents/`（根仓库横切文档） | `Documents/docs/` | push 到 main / 手动 dispatch | `Documents/docs/.vitepress/dist` |
| `Model/shmtu-cas-ocr-model/` | `Model/shmtu-cas-ocr-model/Documents/docs/` | 同上 | `Model/shmtu-cas-ocr-model/Documents/docs/.vitepress/dist` |
| `Lib/shmtu-cas-python/` | `Lib/shmtu-cas-python/Documents/docs/` | 同上 | `Lib/shmtu-cas-python/Documents/docs/.vitepress/dist` |
| `Plugin/shmtu-cas-ocr-crx/` | `Plugin/shmtu-cas-ocr-crx/Documents/docs/` | 同上 | `Plugin/shmtu-cas-ocr-crx/Documents/docs/.vitepress/dist` |

每个站点的工作流统一位于 `<项目>/.github/workflows/docs-pages.yml`（`working-directory: Documents` 指向文档目录）：

- 使用 Node.js 24 + `npm ci` 依赖安装
- `npm run docs:build` 执行 vitepress 构建
- 上传 `Documents/docs/.vitepress/dist` 到 Pages artifact，再由 `actions/deploy-pages@v5` 部署

本地预览：

```bash
cd Documents                 # 或 Model/shmtu-cas-ocr-model/Documents / Lib/shmtu-cas-python/Documents / Plugin/shmtu-cas-ocr-crx/Documents
npm install
...
部署后的 URL 规则（vitepress `resolveBase` 函数自动推导）：

- 组织级 Pages 仓库 (`<org>.github.io`)：根路径 `/`
- 项目级 Pages：`<org>.github.io/<repo-name>/`

修改 `Documents/docs/.md` 后 push 到 main 即触发自动部署。
