...
以下三个项目的 `Documents/` 内已经搭建 **VitePress** 站点并配置了 **GitHub Pages** 部署 workflow（模仿 `shmtu-terminal-tauri` + `shmtu-cas-rs` 的范式）：
...
每个站点的工作流统一位于 `<项目>/Documents/.github/workflows/docs-pages.yml`：
...
```bash
cd Documents                 # 或 Lib/shmtu-cas-python/Documents / Plugin/shmtu-cas-ocr-crx/Documents
npm install
